...
class Rules:
    def __init__(self,name,origin,include_aegis_rule=False):
        self.rules=[]
        self.dirs=[]
        self.origin_dir = origin
        self.package_name = name
        self.include_aegis_rule = include_aegis_rule
...
install: build
    ...
    def footer(self,rules):
        if self.include_aegis_rule:
          aegis_rule = "aegis-deb-add -control debian/modrana/DEBIAN/control .. debian/modrana.aegis=_aegis"
        else:
          aegis_rule = ""
...
binary-arch: build install
	...
	dh_installexamples
	dh_installman
	dh_pysupport $(CURDIR)/debian/modrana/opt/modrana
	dh_link
#	dh_strip # not really needed for Python programs
	dh_compress
	dh_fixperms
	dh_installdeb
#	dh_shlibdeps # not really needed for Python programs
	dh_gencontrol
	dh_md5sums
	dh_builddeb
	%s
...
.PHONY: build clean binary-indep binary-arch binary install configure''' % aegis_rule)
